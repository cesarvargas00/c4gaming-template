...
<script type="text/html" id="cart_template">
	...
	<div class="panel-none <%= parseInt(model.config.display) ? '' : 'hidden' %>">


		<div class="qc-checkout-product" >
			<% if(model.config.description){ %><p class="text">Carrinho de compras<% } %>
			<% if(model.error){ %>
			...
			<table class="table table-bordered qc-cart table-no-border table-cart">
				<thead>
					<tr>
						<td class="qc-image <%= parseInt(model.config.columns.image) ? '' : 'hidden' %>"><?php echo $column_image; ?></td>
						<td class="qc-name <%= parseInt(model.config.columns.name) ? '' : 'hidden' %>"></td>
