		...
		<% if(parseInt(model.config.option.guest.display)){ %>
			<div class="btn-group" role="group">
				<label class="btn <%= model.account == 'guest' ? 'btn-primary active' : 'btn-default' %>">
					<input class="hidden" type="radio" name="account" id="guest" value="guest" autocomplete="off" <%= model.account == 'guest' ? 'checked="checked"' : '' %>> Não criar conta 
				</label>
			</div>
		<% } %>
		<% if(parseInt(model.config.option.register.display)){ %>
			<div class="btn-group" role="group">
				<label  class="btn <%= model.account == 'register' ? 'btn-primary active' : 'btn-default' %> <%= parseInt(model.config.option.register.display) ? '' : 'hidden' %>">
					<input  class="hidden" type="radio" name="account" id="register" value="register" autocomplete="off" <%= model.account == 'register' ? 'checked="checked"' : '' %> > Cadastrar um conta
				</label>
			</div>	
		<% } %>
		<% if(config.design.login_style == 'popup' && parseInt(model.config.option.login.display)){ %> 
			<div class="btn-group" role="group">	
				<button id="login_button_popup" type="button" class="btn btn-default"  data-toggle="modal" data-target="#login_model">Acessar</button>
			</div>
		<% } %>
		</div>
	</div>
	<% } %>
	<% if(model.config.description){ %><p class="description">Ao cadastrar sua conta, você será capaz de comprar mais rápido, ficar atualizado sobre a situação de um pedido e acompanhar o histórico dos pedidos que você já fez.</p><% } %>
